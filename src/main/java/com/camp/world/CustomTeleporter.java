     ...
     private final WorldServer worldServerInstance;
        ...
        public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
        {
                ...
                for (int l = -128; l <= 128; ++l)
                {
                    ...
                    for (int i1 = -128; i1 <= 128; ++i1)
                    {
                        for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
                        {
                            ...
                            if (this.worldServerInstance.getBlockState(blockpos).getBlock() == com.camp.block.BlockManager.customPortalBlock)
                            {
                                while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == BlockManager.customPortalBlock)
                                {
                                    blockpos = blockpos1;
                                }
            ...
            if (d0 >= 0.0D)
            {
                ...
                if (this.worldServerInstance.getBlockState(((BlockPos)object).west()).getBlock() == BlockManager.customPortalBlock)
                {
                    enumfacing = EnumFacing.NORTH;
                }
 
                if (this.worldServerInstance.getBlockState(((BlockPos)object).east()).getBlock() == BlockManager.customPortalBlock)
                {
                    enumfacing = EnumFacing.SOUTH;
                }
 
                if (this.worldServerInstance.getBlockState(((BlockPos)object).north()).getBlock() == BlockManager.customPortalBlock)
                {
                    enumfacing = EnumFacing.EAST;
                }
 
                if (this.worldServerInstance.getBlockState(((BlockPos)object).south()).getBlock() == BlockManager.customPortalBlock)
                {
                ...
                for (i3 = -1; i3 <= 1; ++i3)
                {
                    for (j3 = 1; j3 < 3; ++j3)
                    {
                        for (k3 = -1; k3 < 3; ++k3)
                        {
                            ...
                            j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                            boolean flag = k3 < 0;
                            this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? BlockManager.customBlock.getDefaultState() : Blocks.air.getDefaultState());
                        }
                    }
                }
            }
 
            IBlockState iblockstate = BlockManager.customPortalBlock.getDefaultState();
            ...
            for (j3 = 0; j3 < 4; ++j3)
            {
                for (k3 = 0; k3 < 4; ++k3)
                {
                    for (l3 = -1; l3 < 4; ++l3)
                    {
                        ...
                        this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? BlockManager.customBlock.getDefaultState() : iblockstate, 2);
                    }
                }
