...
public class CustomDungeon extends WorldGenerator
{
    private static final Logger field_175918_a = LogManager.getLogger();
    private static final String[] SPAWNERTYPES = new String[] {"Skeleton", "Zombie", "Enderman", "CustomMob"};
                ...
                for (i2 = 3; i2 >= -1; --i2)
                {
                    for (j2 = i1; j2 <= j1; ++j2)
                    {
                        ...
                        else if (worldIn.getBlockState(blockpos1).getBlock().getMaterial().isSolid() && worldIn.getBlockState(blockpos1).getBlock() != Blocks.chest)
                        {
                            ...
                            {
                                worldIn.setBlockState(blockpos1, Blocks.quartz_block.getStateFromMeta(1), 2);
                            }
                            else
                            {
                                worldIn.setBlockState(blockpos1, Blocks.quartz_block.getStateFromMeta(2), 2);
                            }
                        }
                    }
                }
